...
acqiris::acqiris() : numInstruments_( 0 )
                   , inst_( -1 )
                   , initialized_( false )
                   , simulated_( false )
                   , bus_number_( 0 )
...
acqiris::result_code
acqiris::waitForEndOfAcquisition( size_t timeout ) const
{
...
bool
acqiris::findDevice( bool simulation )
{
    ViStatus status;
    ...
    if ( ( status = AcqrsD1_multiInstrAutoDefine( "cal=0", &numInstruments_ ) ) != VI_SUCCESS ) {
        ...
        if ( numInstruments_ == 0 && simulation ) {
            ...
            if ( Acqrs_InitWithOptions( const_cast<char*>("PCI::DC271")
                                        , VI_FALSE, VI_FALSE, const_cast<char *>("simulate=TRUE")
                                        , &inst_ ) == VI_SUCCESS ) {
                simulated_ = true;
            }
        }
...
acqiris::digitizer_setup( double delay, double width )
{
    ...
    nbrSamples_ = ViInt32( width / 0.5e-9 + 1 );
